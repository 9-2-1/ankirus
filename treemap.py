...
def render_treemap(
    ...
    if root.childs is not None:
        ...
        displays.extend(squarify(root.childs, rect, min_size, level + 1))
    else:
...
def color_from_value(
    value: float, colormap: dict[float, tuple[float, float, float]]
) -> str:
    last = None
    nexx = None
    for v in colormap:
    ...
    oklch = OKLCH(l=cL, c=cC, h=cH)  # type: ignore[no-untyped-call]
...
def svg_from_treemap(
    ...
    for v in colormap.keys():
        svg_content.append(
            f'  <line x1="{w + sidebar_padding_left}" y1="{(v-min_value)/(max_value-min_value)*h}" x2="{w + sidebar_padding_left + sidebar_width}" y2="{(v-min_value)/(max_value-min_value)*h}" stroke="white" stroke-width="1" />'
        )
        svg_content.append(
            f'  <text x="{w + sidebar_padding_left + sidebar_width + 5}" y="{(v-min_value)/(max_value-min_value)*(h-10)+10}" font-size="10" fill="black">{v}</text>'
        )
    for d in displays:
        x, y, w, h = d.rect
        ...
        # Add rectangle
        svg_content.append(
            f'  <rect class="rect" x="{x}" y="{y}" width="{w}" height="{h}" fill="{color}" stroke="{color}" stroke-width="0.5" data-group="{_escape_svg_text(d.text)}" />'
        )
    for d in displays[::-1]:
        if d.level < level_frames and d.level != 0 and not d.leaf:
            ...
            # Add border
            svg_content.append(
                f'  <rect class="rect" x="{x}" y="{y}" width="{w}" height="{h}" fill="none" stroke="white" stroke-width="{(level_frames-d.level)/2}" data-group="{_escape_svg_text(d.text)}" />'
            )

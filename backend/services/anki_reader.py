...
class AnkiCachedReader:
    ...
    def __init__(self, anki_db: str, config: Config) -> None:
        ...
        self.cards: Optional[List[CardModel]] = None
...
def _get_last_review_time(cstats: CardStatsResponse) -> int:
    """获取最后复习时间"""
    time = 0
...
def _get_memory_state(card: Card) -> tuple[float, float, float]:
    """获取记忆状态"""
...
def _calculate_due_time(card: Card, crt: int, tzoffs: int, rollover: int) -> int:
    """计算卡片due时间"""
    from anki.collection import (
        QUEUE_TYPE_NEW,
        QUEUE_TYPE_LRN,

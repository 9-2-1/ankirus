...
from ..models.card import ReplyGroup, ReplyCard, Card
...
async def handle_cards(app: "app.App", request: web.Request) -> web.Response:
    """处理/cards/端点"""
    cards = await app.ankireader.read(
        sanitize=app.sanitizer.sanitize_cached if app.sanitizer else None
    )
...
async def handle_count_due_cards(app: "app.App", request: web.Request) -> web.Response:
    """处理/cards/due/端点"""
    now = int(time.time())
    cards = await app.ankireader.read()
...
def _build_cache_headers(cache_timestamp: float) -> dict[str, Any]:
    """构建缓存相关的响应头"""
...
def _build_response_data(cards: list[Card]) -> list[Union[ReplyGroup, ReplyCard]]:
    """构建响应数据"""
